...
def test_factory_returns_tci_executor():
    ...
    executor = execute_code_factory("tci", provider="together")
...
def test_factory_rejects_tci_for_non_together_provider():
    with pytest.raises(ValueError, match="requires provider 'together'"):
        execute_code_factory("tci", provider="openai")
...
def test_tci_executor_comprehensive():
    ...
    if not os.getenv("TOGETHER_API_KEY"):
        pytest.skip("TOGETHER_API_KEY not set; skipping Together TCI integration test")
...
def test_factory_created_executors_are_callable():
    ...
    tci_executor = execute_code_factory("tci", provider="together")
